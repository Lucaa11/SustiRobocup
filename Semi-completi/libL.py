...
class VL6180X:
    ...
    def __init__(self, address=0x29, debug=False, Id=0, shdnPin=10, i2c=None):
        ...
        GPIO.output(self.shdnPin,GPIO.HIGH)
        time.sleep(0.05)
        
        if self.get_register(self.__VL6180X_SYSTEM_FRESH_OUT_OF_RESET) == 1:
            print ("ToF sensor %d is ready." %self.Id)
            self.ready = True
        else:
            print ("ToF sensor %d reset failure." %self.Id)
            self.ready = False
...
class VL6180Xs:
    def __init__(self, numSens=5, shdnPins=[20,21,16,26,19]):
